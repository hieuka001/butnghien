...
- Nếu chương là phần giữa truyện, cuối chương phải tạo lực kéo sang chương sau.`;
  ...
  let fullText = "";
  try {
    fullText = await streamChat(WRITE_MODEL, WRITER_SYSTEM_INSTRUCTION, prompt, onChunk, 0.78, estimateMaxTokens(targetWords, 3600));
  } catch (error) {
...
${fullText.slice(-3500)}`;
    ...
    onChunk("\n\n");
    fullText += "\n\n";
    fullText += await streamChat(WRITE_MODEL, WRITER_SYSTEM_INSTRUCTION, continuationPrompt, onChunk, 0.72, estimateMaxTokens(Math.max(remainingWords, minWords - countWords(fullText)), 1800));
  }
...
${fullText.slice(-2200)}`;
    onChunk("\n\n");
    fullText += "\n\n";
    fullText += await streamChat(WRITE_MODEL, WRITER_SYSTEM_INSTRUCTION, closingPrompt, onChunk, 0.68, estimateMaxTokens(420, 1200));
  }
...
${userIdea || "Không có bổ sung."}`;
  ...
  let fullText = await streamChat(WRITE_MODEL, WRITER_SYSTEM_INSTRUCTION, prompt, onChunk, 0.82, estimateMaxTokens(targetWords, 3600));
  let rounds = 0;
  const maxContinuationRounds = targetWords <= 2500 ? 6 : targetWords <= 6000 ? 7 : targetWords <= 12000 ? 9 : 12;
...
${fullText.slice(-3500)}`;
    onChunk("\n\n");
    fullText += "\n\n";
    fullText += await streamChat(WRITE_MODEL, WRITER_SYSTEM_INSTRUCTION, continuationPrompt, onChunk, 0.76, estimateMaxTokens(Math.max(remainingWords, minWords - countWords(fullText)), 1800));
  }
...
${fullText.slice(-2200)}`;
    onChunk("\n\n");
    fullText += "\n\n";
    fullText += await streamChat(WRITE_MODEL, WRITER_SYSTEM_INSTRUCTION, closingPrompt, onChunk, 0.7, estimateMaxTokens(420, 1200));
  }
